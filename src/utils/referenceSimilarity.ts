...
const tokenizeDocumentText = (text: string): string[] => {
...
export const rankByTextSimilarity = <T>(
  queryText: string,
  items: readonly T[],
  getDocumentText: (item: T) => string,
): RankedDocument<T>[] => {
  const queryTokens = tokenizeDocumentText(queryText);
  if (queryTokens.length === 0) {
    return items.map((item) => ({ item, score: 0 }));
  }

  const documents = items.map((item) => {
    const tokens = tokenizeDocumentText(getDocumentText(item));
    return {
      item,
      tokens,
      termFrequency: buildTermFrequency(tokens),
